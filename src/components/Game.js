...
export default class Game extends React.Component {
  constructor(props) {
    ...
    db.collection("puyotan")
      ...
      .onSnapshot(docSnapshot => {
        ...
        try {
          cancel0 = db
            .collection(`puyotan/actions/${info.seed}/ids/0`)
        ...
        try {
          cancel1 = db
            .collection(`puyotan/actions/${info.seed}/ids/1`)
            .onSnapshot({ includeMetadataChanges: true }, querySnapshot => {
              let actions = [];
              querySnapshot.forEach(doc => {
                let d = doc.data();
    ...
    window.disableDebug = () => {
      this.setState({ isDebug: false });
    };

    // hash ルーティング用
    let hashChange = hash => {
      let pathBeginPos = hash.indexOf("/");
      if (pathBeginPos === -1) return;
      let pathComponents = hash.substring(pathBeginPos + 1).split("/");
      switch (pathComponents[0]) {
        case "rooms":
          console.log("rooms!", pathComponents[1]);
          break;
        case "replays":
          console.log("replays!", pathComponents[1]);
          break;
        default:
      }
    };
  ...
  render() {
    ...
    let prevAction1 =
      prevHistory1 == null ? new Puyotan.Action() : prevHistory1.action;
    if (
      prevHistory1 != null &&
      prevHistory1.remainingFrame !== 1 &&
      prevAction1.type === Puyotan.ActionType.PUT
    )
      prevAction1 = new Puyotan.Action(Puyotan.ActionType.PUT, 3, 0);
    let prevHistory2 = this.state.actionHistories2[this.state.frame - 1];
    let prevAction2 =
      prevHistory2 == null ? new Puyotan.Action() : prevHistory2.action;
    if (
      prevHistory2 != null &&
      prevHistory2.remainingFrame !== 1 &&
      prevAction2.type === Puyotan.ActionType.PUT
    )
      prevAction2 = new Puyotan.Action(Puyotan.ActionType.PUT, 3, 0);
    ...
    let visibleFloatingPuyo1 =
      this.state.isControlledPlayer1 && currentHistory1 == null;
    let visibleFloatingPuyo2 =
      this.state.isControlledPlayer2 && currentHistory2 == null;

    if (
      currentHistory1 != null &&
      currentHistory1.action.type === Puyotan.ActionType.PUT &&
      currentHistory1.remainingFrame === 0
    ) {
      ...
      currentHistory1 != null &&
      currentHistory1.action.type === Puyotan.ActionType.PUT &&
      currentHistory1.remainingFrame === 1
    ) {
      ...
      currentHistory1 != null &&
      currentHistory1.action.type === Puyotan.ActionType.PASS
    ) {
      floatingDir1 = 0;
      floatingAxisPos1 = new Puyo.Pos(3, 12);
      visibleFloatingPuyo1 = true;
    ...
    if (
      currentHistory2 != null &&
      currentHistory2.action.type === Puyotan.ActionType.PUT &&
      currentHistory2.remainingFrame === 0
    ) {
      ...
      currentHistory2 != null &&
      currentHistory2.action.type === Puyotan.ActionType.PUT &&
      currentHistory2.remainingFrame === 1
    ) {
      ...
      currentHistory2 != null &&
      currentHistory2.action.type === Puyotan.ActionType.PASS
    ) {
      floatingDir2 = 0;
      floatingAxisPos2 = new Puyo.Pos(3, 12);
      visibleFloatingPuyo2 = true;
    ...
    return (
      <div className="Game">
        <div className="Game-pos-chain1">{this.state.chain1} 連鎖</div>
        <div className="Game-pos-chain2">{this.state.chain2} 連鎖</div>
        <div className="Game-pos-score1">{this.state.score1} 点</div>
        <div className="Game-pos-score2">{this.state.score2} 点</div>
        <div className="Game-pos-usedScore1">{this.state.usedScore1} 点</div>
        <div className="Game-pos-usedScore2">{this.state.usedScore2} 点</div>
        ...
        <div className="Game-pos-frame">frame: {this.state.frame}</div>
        ...
        <div
          className={
            "Game-field Game-pos-field1" +
            (this.state.isControlledPlayer1 ? " Game-field-active" : "")
          }
        >
          {this.fieldToElements(this.state.field1)}
          <div
            style={{
              top: 32 * (10 - floatingSubPos1.y),
              left: 32 * (floatingSubPos1.x - 1)
            }}
            className={`Game-puyo ${this.kindToClassName(
              this.state.activePair1.sub
            )} ${visibleFloatingPuyo1 ? "" : "hidden"}`}
          />
          <div
            style={{
              top: 32 * (10 - floatingAxisPos1.y),
              left: 32 * (floatingAxisPos1.x - 1)
            }}
            className={`Game-puyo ${this.kindToClassName(
              this.state.activePair1.axis
            )} ${visibleFloatingPuyo1 ? "" : "hidden"}`}
          />
        ...
        <div
          className={
            "Game-field Game-pos-field2" +
            (this.state.isControlledPlayer2 ? " Game-field-active" : "")
          }
        >
          {this.fieldToElements(this.state.field2)}
          <div
            style={{
              top: 32 * (10 - floatingSubPos2.y),
              left: 32 * (floatingSubPos2.x - 1)
            }}
            className={`Game-puyo ${this.kindToClassName(
              this.state.activePair2.sub
            )} ${visibleFloatingPuyo2 ? "" : "hidden"}`}
          />
          <div
            style={{
              top: 32 * (10 - floatingAxisPos2.y),
              left: 32 * (floatingAxisPos2.x - 1)
            }}
            className={`Game-puyo ${this.kindToClassName(
              this.state.activePair2.axis
            )} ${visibleFloatingPuyo2 ? "" : "hidden"}`}
          />
        ...
        <input
          type="button"
          value={this.state.isControlledPlayer1 ? `[1P] 離席` : `[1P] 着席`}
          onClick={e => this.toggleControlledPlayer1()}
          className={`Game-button Game-size-onoff Game-pos-onoff1`}
        />
        <input
          type="button"
          value={this.state.isControlledPlayer2 ? `[2P] 離席` : `[2P] 着席`}
          onClick={e => this.toggleControlledPlayer2()}
          className={`Game-button Game-size-onoff Game-pos-onoff2`}
        />
        <input
          type="button"
          value="⇦"
          onClick={e => this.moveLeft()}
          className={`Game-button Game-size-button Game-pos-leftmove`}
        />
        <input
          type="button"
          value="⇨"
          onClick={e => this.moveRight()}
          className={`Game-button Game-size-button Game-pos-rightmove`}
        />
        <input
          type="button"
          value="L"
          onClick={e => this.turnLeft()}
          className={`Game-button Game-size-button Game-pos-leftturn`}
        />
        <input
          type="button"
          value="R"
          onClick={e => this.turnRight()}
          className={`Game-button Game-size-button Game-pos-rightturn`}
        />
        <input
          type="button"
          value="⇩"
          onClick={e => this.putPuyo()}
          className={`Game-button Game-size-button2 Game-pos-put`}
        />
        <input
          type="button"
          value="Reset"
          onClick={e => this.sendInit(Number(this.state.seed) + 1)}
          className={`Game-button Game-size-reset Game-pos-reset`}
        />
        <input
          type="number"
          value={this.state.displayedSeed}
          onChange={e => this.setState({ displayedSeed: e.target.value })}
          className={`Game-input-seed`}
        />
        <input
          type="button"
          value="GO"
          onClick={e => this.sendInit(this.state.displayedSeed)}
          className={`Game-button Game-size-go Game-pos-go`}
        />
      </div>
    );
  }
  ...
  getSubPos(axisPos, dir) {
    switch (dir) {
      ...
      case 3:
        return new Puyo.Pos(axisPos.x - 1, axisPos.y);
      default:
        throw Error(`unsupported dir ${this.state.controledDir1}`);
    }
  }
  ...
  actionHistoriesToElements(actionHistories) {
    ...
    for (let index = this.state.frame; index >= 1; index--) {
      ...
      array.push(
        <div key={index} style={style} className={`Game-history-tile`}>
          {index}:{text}
        </div>
      );
    }
  ...
  nextToElements(pair) {
    return [
      <div
        key={"sub"}
        className={`Game-puyo ${this.kindToClassName(pair.sub)}`}
      />,
      <div
        key={"axs"}
        className={`Game-puyo ${this.kindToClassName(pair.axis)}`}
        style={{ top: 32 }}
      />
    ];
  }
  ...
  kindToClassName(kind) {
    switch (kind) {
      case Puyo.Kind.RED:
        return "Game-puyo-kind-red";
      case Puyo.Kind.BLUE:
        return "Game-puyo-kind-blue";
      case Puyo.Kind.GREEN:
        return "Game-puyo-kind-green";
      case Puyo.Kind.YELLOW:
        return "Game-puyo-kind-yellow";
      case Puyo.Kind.OJAMA:
        return "Game-puyo-kind-ojama";
      default:
        return "Game-puyo-kind-blank";
    }
  }
  ...
  moveLeft() {
    if (this.state.isControlledPlayer1)
      if (
        this.state.controledPos1 > 2 ||
        (this.state.controledPos1 === 2 && this.state.controledDir1 !== 3)
      )
        this.setState({ controledPos1: this.state.controledPos1 - 1 });
    if (this.state.isControlledPlayer2)
      if (
        this.state.controledPos2 > 2 ||
        (this.state.controledPos2 === 2 && this.state.controledDir2 !== 3)
      )
        this.setState({ controledPos2: this.state.controledPos2 - 1 });
  }

  moveRight() {
    if (this.state.isControlledPlayer1)
      if (
        this.state.controledPos1 < 5 ||
        (this.state.controledPos1 === 5 && this.state.controledDir1 !== 1)
      )
        this.setState({ controledPos1: this.state.controledPos1 + 1 });
    if (this.state.isControlledPlayer2)
      if (
        this.state.controledPos2 < 5 ||
        (this.state.controledPos2 === 5 && this.state.controledDir2 !== 1)
      )
        this.setState({ controledPos2: this.state.controledPos2 + 1 });
  }
  ...
  turnLeft() {
    if (this.state.isControlledPlayer1) {
      let adjust = 0;
      if (this.state.controledPos1 === 1 && this.state.controledDir1 === 0)
        adjust = 1;
      if (this.state.controledPos1 === 6 && this.state.controledDir1 === 2)
        adjust = -1;
      this.setState({
        controledDir1: (this.state.controledDir1 + 3) % 4,
        controledPos1: this.state.controledPos1 + adjust
      });
    }
    if (this.state.isControlledPlayer2) {
      let adjust = 0;
      if (this.state.controledPos2 === 1 && this.state.controledDir2 === 0)
        adjust = 1;
      if (this.state.controledPos2 === 6 && this.state.controledDir2 === 2)
        adjust = -1;
      this.setState({
        controledDir2: (this.state.controledDir2 + 3) % 4,
        controledPos2: this.state.controledPos2 + adjust
      });
    }
  }
  ...
  turnRight() {
    if (this.state.isControlledPlayer1) {
      let adjust = 0;
      if (this.state.controledPos1 === 1 && this.state.controledDir1 === 2)
        adjust = 1;
      if (this.state.controledPos1 === 6 && this.state.controledDir1 === 0)
        adjust = -1;
      this.setState({
        controledDir1: (this.state.controledDir1 + 1) % 4,
        controledPos1: this.state.controledPos1 + adjust
      });
    }
    if (this.state.isControlledPlayer2) {
      let adjust = 0;
      if (this.state.controledPos2 === 1 && this.state.controledDir2 === 2)
        adjust = 1;
      if (this.state.controledPos2 === 6 && this.state.controledDir2 === 0)
        adjust = -1;
      this.setState({
        controledDir2: (this.state.controledDir2 + 1) % 4,
        controledPos2: this.state.controledPos2 + adjust
      });
    }
  }
  ...
  putPuyo() {
    if (this.state.isControlledPlayer1) {
      this.sendAction(
        ...
        this.state.frame,
        new Puyotan.Action(
          Puyotan.ActionType.PUT,
          this.state.controledPos1,
          this.state.controledDir1
        )
      );
    }
    if (this.state.isControlledPlayer2) {
      this.sendAction(
        ...
        new Puyotan.Action(
          Puyotan.ActionType.PUT,
          this.state.controledPos2,
          this.state.controledDir2
        )
      );
    }
  }
  ...
  pass() {
    if (this.state.isControlledPlayer1) {
      this.sendAction(
        0,
        this.state.frame,
        new Puyotan.Action(Puyotan.ActionType.PASS)
      );
    }
    if (this.state.isControlledPlayer2) {
      this.sendAction(
  ...
  stepNextFrame() {
    ...
    if (
      vm.players[0].actionHistories[vm.frame] != null &&
      vm.players[1].actionHistories[vm.frame] != null
    ) {
  ...
  sendAction(id, frame, action) {
    db.collection(`puyotan/actions/${this.state.seed}/ids/${id}`)
      .doc(String(frame))
      .set({
        type: action.type,
        x: action.x,
  ...
  applyActions(waitMs = 0) {
    ...
    console.log(
      `applyActions(frame, action0, action1)`,
      frame,
      action0,
      action1
    );
